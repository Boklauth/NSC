...
#' @param suffix This is a suffix for the file name. 
#' @return NSC will return a data frame output and at the same time output '.txt' and '.csv' files according to the
  ...
  if(length(unique(data_table_input[,6]))>1){
    ...
    for (i in 1:length(extracted_dates)){
      if (Sys.Date() - extracted_dates[i]< 59){
          ...
          stop(paste("Your latest search date allowed is: ", latest_date,".",
